    ...
    timer.current = setTimeout(async () => {
      try {
        ...
        const url = `https://nominatim.openstreetmap.org/search?q=${encodeURIComponent(
          q
        )}&format=json&addressdetails=1&limit=7&countrycodes=rs`;
        ...
        >
          {results.map((r, i) => (
            <div
              ...
              onMouseEnter={(e) => {
                e.currentTarget.style.background = 'var(--bg-ov)';
              }}
              onMouseLeave={(e) => {
...
export default function PostJobScreen({
  ...
  const handleCreate = useCallback(async () => {
    if (!title.trim()) {
      onMessage('Please enter a job title.', 'error');
      return;
    }
    if (!description.trim()) {
      onMessage('Please describe the work.', 'error');
      return;
    }
    if (!categoryId) {
      onMessage('Please select a category.', 'error');
      return;
    }
    if (!city.trim()) {
      onMessage('Please enter a city.', 'error');
      return;
    }
    if (!address.trim()) {
      onMessage('Please enter an address.', 'error');
      return;
    }
    if (!payPerWorker || Number(payPerWorker) < 1) {
      onMessage('Please enter pay per worker.', 'error');
      return;
    }
    if (!scheduledDate) {
    ...
    if (!canPost) {
      onMessage('Insufficient credits.', 'error');
      return;
    }
  ...
  return (
    <div className="pg-n">
      <div style={{ marginBottom: 24 }}>
        ...
        <p style={{ fontSize: '.9375rem', color: 'var(--tx-2)' }}>
          Describe the work and find skilled help nearby.
        </p>
      </div>
        ...
        <div className="psect">
          ...
          <div className="frow" style={{ gap: 16 }}>
            <div className="fld">
              ...
              <input
                className="inp"
                placeholder="e.g. Help moving furniture to 3rd floor"
                value={title}
            ...
            <div className="loc-info">
              ...
              <div>
                ...
                <div style={{ fontSize: '.75rem', color: 'var(--tx-2)', marginTop: 2 }}>
                  {userLocation
                    ? 'GPS coordinates used for map pin'
                    : 'Allow location for accurate map placement'}
                </div>
              </div>
            </div>
            <button type="button" className="btn btn-s btn-sm" onClick={onRequestLocation}>
        ...
        <div className="psect">
          <div className="cred-notice" style={{ marginBottom: !canPost ? 10 : 16 }}>
            ...
            <span>
              ...
              {!canPost && (
                <span style={{ color: 'var(--err)', marginLeft: 6 }}>Insufficient credits.</span>
              )}
            </span>
          </div>
